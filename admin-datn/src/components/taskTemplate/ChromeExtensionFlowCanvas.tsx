...
import { chromeStepsFromTaskTemplate } from '@/src/lib/taskTemplatePayload';
import { chromeStepsFromWorkflow } from '@/src/lib/workflowGraph';
import {
  CHROME_STEP_PALETTE,
  CHROME_STEPS_MAX,
  actionLabel,
...
import { scriptUrlPattern } from '@/src/lib/workflowGraph/chromeScriptImport';
import {
...
import type { ChromeScript, TaskTemplate, Workflow } from '@/src/types/api';
...
type Props = {
  ...
  urlPattern: string;
  onStepsChange: (steps: ChromeScriptStep[]) => void;
  onUrlPatternChange: (urlPattern: string) => void;
  inspectorFooter?: React.ReactNode;
};

export function ChromeExtensionFlowCanvas({
  compact,
  steps,
  urlPattern,
  onStepsChange,
  ...
  useEffect(() => {
    const { nodes: nextNodes, edges: nextEdges } = buildLinearRecordingFlow(flowInputs, 'chrome', {
  ...
  const addStep = (action: ChromeScriptAction) => {
    if (steps.length >= CHROME_STEPS_MAX) return;
  ...
  const updateStep = (id: string, patch: Partial<ChromeScriptStep>) => {
  ...
  const applyChromeSteps = (imported: ChromeScriptStep[], urlPat?: string) => {
    if (imported.length === 0) return;
    if (urlPat) onUrlPatternChange(urlPat);
    updateSteps(imported.slice(0, CHROME_STEPS_MAX));
    setSelectedStepId(imported[0]?.id ?? null);
    setPropertiesOpen(true);
  };

  const importScript = (script: ChromeScript) => {
    const imported = parseStepsFromJson(script.steps);
    const pat = scriptUrlPattern(script);
    applyChromeSteps(imported, pat);
  };

  const importFromTaskTemplate = (template: TaskTemplate) => {
    const { steps, urlPattern } = chromeStepsFromTaskTemplate(template);
    applyChromeSteps(steps, urlPattern || undefined);
  };

  const importFromWorkflow = (workflow: Workflow) => {
    const { steps, urlPattern } = chromeStepsFromWorkflow(workflow);
    applyChromeSteps(steps, urlPattern || undefined);
  };
    ...
    (_: React.MouseEvent, dragged: Node<RecordingFlowNodeData>) => {
      setNodes((currentNodes) => {
        ...
        const reordered = orderIds
          ...
          .filter((s): s is ChromeScriptStep => Boolean(s));
  ...
  const paletteBody = (
    <>
      <WfImportMenu
        compact
        onImportChromeScript={importScript}
        onImportDesktopRecording={() => {}}
        onImportTaskTemplate={importFromTaskTemplate}
        onImportWorkflow={importFromWorkflow}
      />
      <div className="space-y-2">
        {CHROME_STEP_PALETTE.map(({ action, iconName }) => {
          const Icon = PALETTE_ICONS[iconName];
          const atMax = steps.length >= CHROME_STEPS_MAX;
          return (
            <button
              key={action}
              type="button"
  ...
  const inspectorTitle = selectedStep
    ...
    : t('templateWizard.chromeConfigMeta');
    ...
    <>
      <div className={cn(selectedStep && 'pb-4 border-b border-white/5')}>
        ...
        <input
          ...
          className="w-full mt-1 px-3 py-2 rounded-xl bg-surface-container-low border border-white/10 font-mono text-sm"
        />
        <p className="text-[10px] text-on-surface-variant mt-1.5">
          {t('templateWizard.chromeStepCount', {
            count: String(steps.length),
            max: String(CHROME_STEPS_MAX),
          })}
        </p>
      </div>
      {selectedStep ? (
      ...
      <div className="flex-1 min-w-0 min-h-0 flex flex-col relative">
        ...
        {isCompact ? (
          <button
            ...
            className="absolute left-3 bottom-3 z-10 inline-flex items-center gap-1.5 px-3 py-2 rounded-full bg-surface-container-high/95 border border-white/10 text-[10px] font-bold shadow-lg"
          >
            <Layers size={14} />
            {t('chromeScripts.palette')}
          </button>
