...
import { ChromeScriptStepInspector } from './ChromeScriptStepInspector';
...
type Props = {
  ...
  onChange: (steps: ChromeScriptStep[]) => void;
  readOnly?: boolean;
};

export function ChromeScriptStepBuilder({ steps, onChange, readOnly = false }: Props) {
  ...
  const updateSteps = (next: ChromeScriptStep[]) => {
    onChange(next);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {steps.length === 0 ? (
          ...
          <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar py-2">
            ...
            {steps.map((step, idx) => {
              const pal = CHROME_STEP_PALETTE.find((p) => p.action === step.action);
              const Icon = pal ? ICONS[pal.iconName] : Clock;
              return (
                <React.Fragment key={step.id}>
                  ...
                  >
                    ...
                    <GripVertical
                      size={16}
                      className="text-on-surface-variant/40 shrink-0 cursor-grab"
                    />
                    ) : null}
                    ...
                    <button
                      ...
                      title={t('chromeScripts.deleteStep')}
                    >
                      <Trash2 size={16} />
                    </button>
                    ) : null}
      ...
      >
        ...
        />
        <details className="mt-6 shrink-0">
          <summary className="text-[10px] font-mono font-bold uppercase text-on-surface-variant cursor-pointer">
            {t('chromeScripts.jsonPreview')}
          </summary>
          <pre className="mt-2 text-[10px] font-mono bg-[#0b0f14] p-3 rounded-lg border border-white/10 max-h-48 overflow-auto text-[#d4d4d4]">
            {previewJson}
          </pre>
        </details>
      </div>
