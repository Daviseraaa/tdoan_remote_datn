# Yêu cầu Admin Console (StationHub)

Tài liệu mô tả **chức năng**, **phân quyền** và **API** mà ứng dụng `admin-stationhub/` (StationHub Console) gọi tới backend NestJS. Base URL REST: `VITE_API_BASE_URL` (mặc định `http://localhost:3000/api`). Mọi response thành công bọc trong `{ success, data, timestamp }` — frontend dùng `unwrap()` để lấy `data`.
...
Logic path: `admin-stationhub/src/lib/apiScope.ts` — `isAdmin` chọn prefix.
...
|------------|-----------|
| Routes | `admin-stationhub/src/App.tsx` |
| API scope admin vs tenant | `admin-stationhub/src/lib/apiScope.ts` |
| Backend admin | `src/modules/admin/admin.controller.ts` |
| Trang UI | `admin-stationhub/src/views/*.tsx` |
