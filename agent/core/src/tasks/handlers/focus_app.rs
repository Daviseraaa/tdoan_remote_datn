        ...
        let payload = match payload_object(task) {
            ...
            None => {
                return (
                    false,
                    -1,
                    Some("FOCUS_APP: thiếu payload (pid/processName/windowTitle)".into()),
                    None,
                );
            }
        };

        match focus_app::focus_execute(&payload).await {
            Ok(r) => {
                let ok = !r.focused.is_empty() || r.failed.is_empty();
                (
                    ok,
                    if ok { 0 } else { -1 },
                    if ok {
                        ...
                        Some("Không focus được app/cửa sổ nào".into())
                    },
                    Some(json!({
                        "focused": r.focused,
                        "failed": r.failed,
