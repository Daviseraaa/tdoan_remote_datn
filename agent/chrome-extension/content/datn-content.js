...
(function () {
  ...
  function shouldSkipRecordTarget(el) {
    ...
    if (el.closest('[data-datn-no-record]')) return true;
    if (el.closest('#datn-recorder-badge')) return true;
    if (el.closest('#datn-recorder-hover-root')) return true;
  ...
  function ensureHoverUI() {
    ...
    root.id = 'datn-recorder-hover-root';
    root.setAttribute('data-datn-no-record', '1');
    root.style.cssText = 'position:fixed;inset:0;z-index:2147483645;pointer-events:none;';

    const box = document.createElement('div');
    box.id = 'datn-recorder-hover-box';
    box.style.cssText =
    ...
    label.id = 'datn-recorder-hover-label';
    label.style.cssText =
  ...
  function teardownHoverUI() {
    ...
    const root = document.getElementById('datn-recorder-hover-root');
    if (root) root.remove();
    hoverBoxEl = null;
    hoverLabelEl = null;
  }
  ...
  function updateRecorderBadge() {
    let badge = document.getElementById('datn-recorder-badge');
    if (!recording) {
    ...
    if (!badge) {
      ...
      badge.id = 'datn-recorder-badge';
      badge.setAttribute('data-datn-no-record', '1');
      badge.style.cssText =
    ...
    badge.textContent = `DATN ghi: ${recordedSteps.length} bước`;
  }
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg?.type === 'datn-record') {
      runRecordAction(msg.action)
        .then((data) => sendResponse({ ok: true, data }))
        .catch((e) => sendResponse({ ok: false, error: e?.message || String(e) }));
      return true;
    }
    if (msg?.type === 'datn-replay') {
      try {
